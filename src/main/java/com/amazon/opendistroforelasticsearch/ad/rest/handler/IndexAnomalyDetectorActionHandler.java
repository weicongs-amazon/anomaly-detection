...
public class IndexAnomalyDetectorActionHandler extends AbstractActionHandler {
    ...
    private void validateCategoricalField(String detectorId) {
        ...
        if (categoryField.size() != 1) {
            onFailure(new IllegalArgumentException(ONLY_ONE_CATEGORICAL_FIELD_ERR_MSG));
            return;
        }
        ...
        ActionListener<GetFieldMappingsResponse> mappingsListener = ActionListener.wrap(getMappingsResponse -> {
                        ...
                        if (fieldMetadata != null) {
                            ...
                            if (metadata != null && metadata instanceof Map) {
                                ...
                                if (!typeName.equals(CommonName.KEYWORD_TYPE) && !typeName.equals(CommonName.IP_TYPE)) {
                                    onFailure(new IllegalArgumentException(CATEGORICAL_FIELD_TYPE_ERR_MSG));
                                    return;
                                }
                            }
                        }
            ...
            if (foundField == false) {
                onFailure(new IllegalArgumentException(String.format(NOT_FOUND_ERR_MSG, categoryField0)));
                return;
            }
